...
func init() {
    ...
    if _, err := rand.Read(b); err == nil {
        sessionCookieSalt = fmt.Sprintf("%x", b)
    } else {
        sessionCookieSalt = "fallback_salt"
    }
}
...
func getSessionCookieName(pl_name string, cookie_name string) string {
    data := cookie_name + ":" + pl_name + ":" + sessionCookieSalt
    hash := sha256.Sum256([]byte(data))
    s := fmt.Sprintf("%x", hash[:6])           // 12 hex chars
    return cookie_name + "_" + s              // e.g. sess_9af31c2b77d4
}